...
import {getEarn, getEarnReturn} from '../../hooks/api/get-api';
import {useAppSelector} from '../../hooks/redux';
import colors from '../../styles/colors';
import NoData from '../../components/NoData';

let lastDoc: any = 1;
let lastDocReturn: any = 1;
const EarnInvoices = (props: any) => {
  const {is_return} = props.route;
  ...
  const getMore = async () => {
    ...
    if ((is_return ? lastDocReturn : lastDoc) > 0) {
      setIsMoreLoading(true);
      let result = await fetchData((is_return ? lastDocReturn : lastDoc) + 1);
      let data: any = [];
      ...
      if (is_return)
        lastDocReturn = Math.ceil(data.length / result?.result_per_page);
      else lastDoc = Math.ceil(data.length / result?.result_per_page);
      if (Number(result?.total) <= data.length) {
        if (is_return) lastDocReturn = 0;
        else lastDoc = 0;
      }
  ...
  const fetchData = async (page: number = 1) => {
    let result: any = null;
    if (is_return) {
      result = await getEarnReturn({
        customer_id: user?.id,
        page,
      });
    } else {
      result = await getEarn({
        customer_id: user?.id,
        page,
      });
    }
  ...
  const renderInvoiceItem = useCallback(({item}: any) => {
    return <PointItem item={item} is_background={false} />;
  }, []);
    ...
    <View
      ...
      {invoices === null ? (
        ...
        <FlatListVertical
          ...
          ListFooterComponent={
            isMoreLoading &&
            (is_return ? lastDocReturn : lastDoc) !== 0 &&
            renderFooter()
          }
          data={invoices}
